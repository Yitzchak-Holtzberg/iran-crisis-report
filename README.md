...
**Decision: No external CSS framework.** The custom CSS in `css/` is already a purpose-built design system that uses CSS custom properties (`--accent-red`, `--bg-card`, etc.) for design tokens, and a layered file structure (`variables.css → base.css → components.css → sidebar.css → responsive.css`). The dashboard's highly specific dark/light news-feed aesthetic and the tight integration with Leaflet map icons would not translate cleanly to a utility framework like Bootstrap or Tailwind — adding one would only introduce naming conflicts and extra bundle weight without simplifying any existing code.

The existing CSS is the right approach for this project. Use `css/variables.css` for all colour and font changes.

### JavaScript framework

**Decision: Alpine.js (v3.14.9) added.** The site's interactive behaviour (sidebar collapse/expand, mobile slide-in panel, dark/light theme toggle) previously required ~140 lines of imperative DOM-manipulation code scattered across `js/sidebar.js` and `js/theme.js`. These are a natural fit for [Alpine.js](https://alpinejs.dev/), a minimal (~40 KB CDN) declarative framework that lives entirely in HTML attributes.

**What changed:**

| Before | After |
|---|---|
| `onclick="toggleCollapse()"` global function | `@click="toggleCollapse()"` — Alpine calls the method |
| `onclick="toggleSidebar()"` + `closeSidebar()` | `@click="toggleMobile()"` / `@click="closeMobile()"` |
| `innerHTML = '▶'/'◀'` in JS | `x-text="sidebarCollapsed ? '▶' : '◀'"` |
| `classList.toggle('open', …)` in JS | `:class="{ open: mobileOpen }"` — Alpine manages it |
| `onclick="toggleTheme()"` global function | `@click="toggleTheme()"` — Alpine calls the method |
| `textContent = '🌙'/'☀️'` in theme.js | `x-text="theme === 'light' ? '🌙' : '☀️'"` |
| `theme.js` (state + Leaflet swap) | Merged into `js/app.js` (Alpine component) |
| `sidebar.js` (toggle + scroll) | `sidebar.js` (scroll tracking only) |

The Alpine component is defined in **`js/app.js`** via a `pageApp()` function. It is loaded at the bottom of `<body>` (below Leaflet), and `x-data="pageApp()"` is set on `<body>` in `sections/head-base.html`. Scroll tracking and back-to-top remain as vanilla JS in `js/sidebar.js`.

A small inline `<script>` in `<head>` restores the saved theme from `localStorage` before the first paint, eliminating any flash of unstyled content for users who have selected light mode.

**Leaflet.js** (`js/map.js`) is unaffected — it is still the correct and only map library needed.

**React/Vue/Angular** would be vastly over-engineered for a mostly-static news dashboard where state management consists of three boolean values.

---

## Quick data patch (no clone required)

For urgent corrections between scheduled AI updates, use the **Quick Data Patch** GitHub Actions workflow. It lets you update any `data.json` field directly from the GitHub UI, triggers a full rebuild, and deploys to Pages — all without cloning the repository or touching the AI pipeline.

**How to use it:**

1. Go to **Actions → Quick Data Patch → Run workflow**.
2. Fill in only the fields you want to change; leave the rest blank.
3. For the **Ticker** field, separate multiple headlines with a `|` pipe character:
   ```
   HEADLINE ONE|HEADLINE TWO|HEADLINE THREE
   ```
4. Click **Run workflow**.

The workflow calls `scripts/patch-data.js`, which validates every field against `data.schema.json` before writing, then runs `npm run build` and deploys.

**From the command line**, you can also run the patcher directly:

```bash
node scripts/patch-data.js --statIrgcKilled="450+" --lastUpdated="15:30 UTC"
node scripts/patch-data.js --ticker="BREAKING: Headline 1|UPDATE: Headline 2"
npm run build
```

Only keys defined in `data.schema.json` are accepted; unknown keys are rejected with an error.
