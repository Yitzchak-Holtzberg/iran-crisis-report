...
├── reactions.html      # GENERATED — redirect to diplomatic.html#reactions-iran
...
│   ├── reactions-teaser.html   # Reactions teaser card (links to diplomatic.html#reactions-iran)
...
`build.js` processes a `BUILDS` array of page definitions, each specifying an output file and its list of section files. It generates **7 HTML pages**: `index.html`, `diplomatic.html` (which now includes the reactions content), `scenarios.html`, `forces.html`, `inside-iran.html`, `reactions.html` (a redirect to `diplomatic.html#reactions-iran`), and `sources.html`.
